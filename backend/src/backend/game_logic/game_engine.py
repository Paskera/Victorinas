...
class DeathPartyGameEngine:
    ...
    def _initialize_questions(self):
        ...
        self.questions_pool = [
            ...
            Question(id=str(uuid.uuid4()), text="Что бы вы предпочли: иметь вместо пальцев сосиски или вместо носа морковку?", category="Абсурд"),
            Question(id=str(uuid.uuid4()), text="Какой самый бесполезный предмет вы бы добавили в капсулу времени для будущих поколений?", category="Разное"),
            Question(id=str(uuid.uuid4()), text="Если бы ваш питомец вдруг стал вашим начальником, что бы он вменил вам в первую очередь?", category="Животные"),
            Question(id=str(uuid.uuid4()), text="Какой суперсилой вы бы НЕ хотели обладать, даже если бы её предлагали?", category="Фантазия"),
            Question(id=str(uuid.uuid4()), text="Что бы вы стали делать, если бы обнаружили, что ваш холодильник каждую ночь уходит на прогулку?", category="Абсурд"),
            Question(id=str(uuid.uuid4()), text="Какой самый странный комплимент вам когда-либо говорили?", category="Истории"),
            Question(id=str(uuid.uuid4()), text="Если бы овощи могли говорить, какой из них был бы самым заносчивым и почему?", category="Еда"),
            Question(id=str(uuid.uuid4()), text="Что самое нелепое можно сделать с обычным носком, если очень постараться?", category="Фантазия"),
            Question(id=str(uuid.uuid4()), text="Какой самый смешной способ провалить собеседование вы можете себе представить?", category="Разное"),
            Question(id=str(uuid.uuid4()), text="Если бы вы создавали новый праздник, какой самый бесполезный, но весёлый повод для него придумали бы?", category="Абсурд")
        ]
